...
- 项目主卡显示最近 3 条执行动态；工具调用统一以 `🛠️` 标识，并额外显示最新一段精简消息。
- Request 卡保存原始请求、执行计划、步骤状态和用时，不重复中间回答。
- Answer 卡在同一张卡片中持续更新累计中间消息；内容过长时保留最新窗口，完成后保留最终回答。
- TraeX 原生状态栏和 Todo 列表不会重复出现在 Answer 卡中，结构化步骤统一由 Request 卡展示。
