...
function setup(operatorOpenIds: readonly string[] = []) {
  ...
  const create = (id: string, role: "primary" | "worker") => store!.createAgentInstance({ id, projectId: "p1", name: id, role, agentKind: "traex", model: null, desiredState: "stopped", workspace: { id: `ws-${id}`, kind: role === "primary" ? "main-checkout" : "shared-read-only", cwd: "/repo", branch: null, baseCommit: "base" } });
  ...
  const workflow = new InstanceInteractionWorkflow({ projects: [project], operatorOpenIds, store, control: control as never, messaging: messaging as never, drivers: { describe: () => ({ available: true, structuredEvents: true, nativeResume: true, primaryTools: true, steering: "terminal-input", interrupt: "terminal-signal", approvals: "terminal", modelSelection: "runtime", usageReporting: true }) } as never, outbound: outbound as never });
...
describe("instance routing", () => {
  it("shows the instance directory instead of choosing when no primary exists", async () => {
    const { workflow, outbound } = setup(); store!.setConversationTarget({ chatId: "chat", projectId: "p1", target: { kind: "primary" } });
  ...
  it("keeps a persistent target but /to remains a one-shot destination", async () => {
    ...
    store!.setConversationTarget({ chatId: "chat", projectId: "p1", target: { kind: "primary" } });
    ...
    expect(store!.getConversationTarget("chat")).toEqual({ projectId: "p1", target: { kind: "primary" } });
