...
describe("PaneRetentionWorkflow", () => {
  it("closes an eligible opted-in pane and persists the operation", async () => {
    ...
    const workflow = new PaneRetentionWorkflow({ projects: [project], store: store as never, herdr: { getPane: vi.fn(async () => present ? pane : null), closePane: close }, isBindingBusy: () => false, logger: { info: vi.fn(), warn: vi.fn() } });
    await workflow.scan();
  ...
  it("does not repeat an unresolved automatic close", async () => {
    ...
    const workflow = new PaneRetentionWorkflow({ projects: [project], store: store as never, herdr: { getPane: vi.fn(async () => pane), closePane: close }, isBindingBusy: () => false, logger: { info: vi.fn(), warn: vi.fn() } });
    await workflow.scan();
  ...
  it("does not close when runtime identity no longer matches the binding", async () => {
    ...
    const close = vi.fn(async () => undefined);
    const workflow = new PaneRetentionWorkflow({ projects: [project], store: store as never, herdr: { getPane: vi.fn(async () => ({ ...pane, terminalId: "different-session" })), closePane: close }, outbound: { enqueueCard: vi.fn(async () => undefined) }, isBindingBusy: () => false, logger: { info: vi.fn(), warn: vi.fn() } });
    await workflow.scan();
    expect(close).not.toHaveBeenCalled();
    expect(store.createAutomaticPaneCloseOperation).not.toHaveBeenCalled();
  });
