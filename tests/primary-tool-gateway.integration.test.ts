...
describe("Primary tool gateway", () => {
  ...
  it("derives the parent turn server-side and rejects forged or stale credentials", async () => {
    ...
    await expect(call(socketPath, { bindingId: "binding", generation: 1, capability, tool: "inspectInstance", arguments: { instanceId: "legacy-primary" } })).resolves.toMatchObject({ ok: false, error: expect.stringMatching(/only same-project workers/) });
