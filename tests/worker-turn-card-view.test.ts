...
describe("WorkerTurnCardView", () => {
  it("creates a queued projection with stable delivery identity", () => {
    expect(queued()).toEqual({
      ...
      phase: "queued", requestText: "Review the transaction boundary", answer: "", timelineItems: [], statusTitle: null, tokenCount: null, progressEvents: [], progressSummary: { total: 0, stepTotal: 0, stepDone: 0 }, queuePosition: 2,
  ...
  it("terminalizes an incomplete Tool without losing its timeline position", () => {
    const running = reduceWorkerTurnCard(queued(), { type: "output", occurredAt: "running", answer: "", timelineDeltas: [
      { kind: "tool", id: "tool:1", sequence: 1, category: "command", label: "build", state: "running" }
    ] });
    const completed = reduceWorkerTurnCard(running, { type: "completed", occurredAt: "done", answer: "done" });

    expect(completed.timelineItems).toEqual([{ kind: "tool", id: "tool:1", sequence: 1, category: "command", label: "build", state: "failed" }]);
  });
});
