...
const definitions = [
  ...
  tool("show_worker_cards", "Queue fresh snapshot cards for one exact-name Worker owned by this Primary: Worker Main first, then its latest Task. This never prompts or controls the Worker.", required({ workerName: stringField("Exact Worker display name."), idempotencyKey: stringField("Stable unique key for this intended display request.") }))
];
