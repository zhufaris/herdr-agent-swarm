...
const definitions = [
  ...
  tool("follow_up_instance", "Queue a follow-up turn for an existing same-project Worker. It runs in FIFO order and does not imply steering.", required({ instanceId: stringField("Worker instance ID."), text: stringField("Follow-up instruction."), idempotencyKey: stringField("Stable unique key for this intended submission.") })),
...
const methodNames: Record<string, string> = { list_instances: "listInstances", prompt_instance: "promptInstance", follow_up_instance: "followUpInstance", steer_instance: "steerInstance", inspect_instance: "inspectInstance", wait_instance: "waitInstance", interrupt_instance: "interruptInstance" };
