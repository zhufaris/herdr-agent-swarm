...
function redactCommandArgs(args: string[]): string[] {
  ...
  if (safe[0] === "agent" && safe[1] === "model-prompt" && safe[2] === "commit" && safe.length > 4) safe[4] = "[REDACTED]";
...
function sensitiveCommandArgValues(args: string[]): string[] {
  ...
  if (args[0] === "agent" && args[1] === "model-prompt" && args[2] === "commit" && args.length > 4) values.push(args[4]!);
