...
function parseNode(value: unknown): GatewayViewNode {
  ...
  if (tag === "button") {
    ...
    return { kind: "button", label: textContent(record(node.text, "button text")) ?? "Action", ...(buttonStyle(node.type) ? { style: buttonStyle(node.type)! } : {}), action: structuredClone(behavior.value), ...(typeof node.name === "string" ? { name: node.name } : {}), ...(node.action_type === "form_submit" ? { submit: true } : {}) };
  }
...
function materializeNode(node: GatewayViewNode): object {
  ...
  if (node.kind === "button") return { tag: "button", text: { tag: "plain_text", content: node.label }, ...(node.style ? { type: node.style } : {}), ...(node.name ? { name: node.name } : {}), ...(node.submit ? { action_type: "form_submit" } : {}), behaviors: [{ type: "callback", value: structuredClone(node.action) }] };
