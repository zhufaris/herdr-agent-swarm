...
export class DeterministicNaturalLanguageCommandInterpreter implements NaturalLanguageCommandInterpreter {
  ...
  interpret(rawText: string): NaturalLanguageCommandResult {
    const context = { text: normalize(rawText), projects: this.projects };
    if (!context.text) return { outcome: "task", source: "deterministic" };
    for (const rule of ORDERED_RULE_GROUPS) {
...
interface InterpretationContext { text: string; projects: readonly ProjectConfig[]; }
type InterpretationRule = (context: InterpretationContext) => NaturalLanguageCommandResult | null;

const ORDERED_RULE_GROUPS: readonly InterpretationRule[] = [
  unsupportedControlRule, exactQueryRule, ambiguityRule, projectAndPrimaryRule, workerRule,
  currentSessionMutationRule, taskClassificationRule
];

function unsupportedControlRule({ text }: InterpretationContext): NaturalLanguageCommandResult | null {
  return /^(?:(?:创建|新建)(?:一个)?(?:新)?(?:项目|workspace|space)|(?:create|add)\s+(?:(?:a|one)\s+)?(?:new\s+)?(?:project|workspace|space))(?:\s|$)/i.test(text)
    ? unsupported("飞书自然语言不能新增项目或 Herdr Workspace；项目来自受控的 projects.json。你可以查看已配置项目，或在项目中创建新的 Primary 任务/会话。", ["查看项目", "在 <项目> 创建新任务：<标题>"])
    : null;
}

function exactQueryRule({ text }: InterpretationContext): NaturalLanguageCommandResult | null {
  if (/^(?:帮助|查看帮助|help|commands?)$/i.test(text)) return swarm({ kind: "help" });
  if (/^(?:(?:查看|显示|列出)\s*(?:所有)?\s*(?:已配置)?\s*项目|(?:show|list)?\s*projects?)$/i.test(text)) return swarm({ kind: "projects" });
  if (/^(?:(?:查看|显示|列出)\s*(?:所有)?\s*(?:space|空间)|(?:show|list)?\s*spaces?)$/i.test(text)) return swarm({ kind: "spaces" });
  if (/^(?:(?:查看|显示|列出)\s*(?:所有)?\s*(?:pane|面板)|(?:show|list)?\s*panes?)$/i.test(text)) return swarm({ kind: "panes" });
  if (/^(?:(?:查看|显示|列出)\s*(?:所有)?\s*会话|(?:show|list)?\s*sessions?)$/i.test(text)) return swarm({ kind: "sessions", cursor: null });
  if (/^(?:(?:查看|显示|列出)\s*(?:所有)?\s*(?:失败|故障)|(?:show|list)?\s*failures?)$/i.test(text)) return swarm({ kind: "failures" });
  if (/^(?:(?:查看|显示)?(?:当前)?状态(?:怎么样|如何)?|(?:show\s+)?status|当前状态怎么样|现在怎么样)$/i.test(text)) return swarm({ kind: "status" });
  if (/^(?:(?:查看|显示)(?:当前)?模型|(?:show\s+)?model)$/i.test(text)) return swarm({ kind: "model", name: null });
  if (/^(?:(?:查看|显示|列出)\s*(?:所有)?\s*(?:workers?|实例|工作者)|(?:show|list)?\s*(?:workers?|instances?))$/i.test(text)) return instance({ kind: "instances" });
  return null;
}

function ambiguityRule({ text }: InterpretationContext): NaturalLanguageCommandResult | null {
  return /^(?:停一下|停止|stop|换个模型|切换模型|创建\s*worker|新建\s*worker|创建任务|新建任务)$/i.test(text)
    ? clarify("这条控制指令缺少必要的目标或参数，不会执行。", ["停止当前任务", "切换模型 <model>", "创建 worker <name>"])
    : null;
}

function projectAndPrimaryRule({ text, projects }: InterpretationContext): NaturalLanguageCommandResult | null {
  const project = /^(?:(?:切换|选择|打开)(?:到)?项目|(?:switch(?:\s+to)?|select|open)\s+project)\s+(.+)$/i.exec(text);
  if (project) {
    const resolved = projectId(project[1]!, projects);
    return resolved ? instance({ kind: "project", projectId: resolved }) : clarify("没有找到这个已配置项目。", ["查看项目", "选择项目 <project-id>"]);
  }
  const primary = /^(?:在\s+(.+?)\s+)?(?:(?:创建|新建)(?:一个)?(?:新)?(?:primary)?(?:任务|会话)|(?:create|start)\s+(?:(?:a|one)\s+)?(?:new\s+)?(?:primary\s+)?(?:task|session))(?:\s*[:：-]\s*|\s+)?(.*)$/i.exec(text);
  if (!primary) return null;
  if (primary[1] && !projectId(primary[1], projects)) return clarify("没有找到目标项目。", ["查看项目", "在 <project-id> 创建新任务：<标题>"]);
  return swarm({ kind: "new", title: primary[2]?.trim() || null, agentKind: "traex" });
}

function workerRule({ text }: InterpretationContext): NaturalLanguageCommandResult | null {
  ...
  return workerOpen ? instance({ kind: "instance", name: workerOpen[1]! }) : null;
}

function currentSessionMutationRule({ text }: InterpretationContext): NaturalLanguageCommandResult | null {
...
function taskClassificationRule({ text }: InterpretationContext): NaturalLanguageCommandResult | null {
  if (TASK_LEADS.test(text)) return { outcome: "task", source: "deterministic" };
  return COMMAND_LEADS.test(text) ? clarify("无法唯一确定要执行的 Swarm 指令，不会作为 Agent 任务发送。", ["查看帮助", "查看状态", "创建新任务：<标题>"]) : null;
}

function projectId(value: string, projects: readonly ProjectConfig[]): string | null {
  const normalized = value.trim().toLowerCase();
  const matches = projects.filter((project) => project.id.toLowerCase() === normalized || project.displayName.toLowerCase() === normalized || project.spaceName?.toLowerCase() === normalized);
...
function swarm(value: BridgeCommand): NaturalLanguageCommandResult { return command({ family: "swarm", command: value }); }
function instance(value: InstanceCommand): NaturalLanguageCommandResult { return command({ family: "instance", command: value }); }
