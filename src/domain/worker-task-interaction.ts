...
export function workerTaskInteraction(phase: WorkerTurnCardPhase): WorkerTaskInteraction {
  ...
  if (phase === "blocked") return { replyIntent: "steer", actionLabel: "补充当前任务", guidance: "Worker 正等待 Herdr 中的本地处理；可补充要求，但审批仍须在对应 Pane 完成。", canInterrupt: false };
  ...
  if (phase === "dispatch-uncertain") return { replyIntent: "reject", actionLabel: null, guidance: "请求可能已到达 Worker。为避免重复执行，请先在对应 Herdr Pane 核实；这里不会重试或追加。", canInterrupt: false };
  if (phase === "queued") return { replyIntent: "reject", actionLabel: null, guidance: "任务尚未开始，暂不能追加；可等待调度，或从 Worker Main 发起独立新任务。", canInterrupt: false };
