...
export interface TopicViewState {
  bindingId: string; bindingGeneration: number; title: string; agentKind: AgentKind; workspaceId: string; spaceName: string; tabId: string | null; paneId: string | null; worktreeName: string | null; phase: TopicViewPhase;
...
function reduceTopicViewSnapshot(state: TopicViewState, event: BridgeEvent): TopicViewState {
  ...
  switch (event.type) {
    case "BindingCreated": return { ...base, title: event.payload.title, agentKind: event.payload.agentKind ?? base.agentKind, workspaceId: event.payload.workspaceId, spaceName: event.payload.spaceName ?? base.spaceName, tabId: event.payload.tabId ?? base.tabId, paneId: event.payload.paneId, phase: "provisioning" };
...
function sameTopicPresentation(left: TopicViewState, right: TopicViewState): boolean {
  return left.bindingGeneration === right.bindingGeneration && left.title === right.title && left.agentKind === right.agentKind && left.workspaceId === right.workspaceId && left.spaceName === right.spaceName
