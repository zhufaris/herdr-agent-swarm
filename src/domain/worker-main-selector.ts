...
export interface WorkerMainProjectionSource {
  ...
  lifecycle: "active" | "legacy" | "terminated"; parentBindingId: string; parentBindingGeneration: number; parentPaneId: string; ownerName: string;
...
export function selectWorkerMainView(source: WorkerMainProjectionSource, previous: WorkerMainView | null, dependencyRevision: number, occurredAt: string): WorkerMainView {
  const base = previous ?? createWorkerMainView({
    ...
    workerName: source.workerName, ownerName: source.ownerName, runtimeGeneration: source.runtimeGeneration, runtimeState: source.runtimeState, paneId: source.paneId, workspace: source.workspace, branch: source.branch, model: source.model, occurredAt: source.createdAt
  });
  ...
  const runtime = reduceWorkerMainView(base, { type: "runtime", runtimeGeneration: source.runtimeGeneration, runtimeState: source.runtimeState, paneId: source.paneId, occurredAt });
