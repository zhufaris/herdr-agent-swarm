...
interface Options {
  ...
  presentation: Pick<ApplicationPresentation, "answerCard" | "instanceCreate" | "instanceDetail" | "instanceDirectory" | "instanceRemovalPlan" | "instanceSteer" | "mainCard" | "requestRejected" | "workerMain" | "workerNewTask" | "workerTaskInstruction" | "workerTurn">;
...
export class InstanceInteractionWorkflow {
  ...
  constructor(private readonly options: Options) {
    ...
    this.workerLifecycle = new WorkerLifecycleActions({ projects: options.projects, store: options.store, control: options.control, messaging: options.messaging, context: this.context, views, presentation: options.presentation, ...(options.workerCreation ? { workerCreation: options.workerCreation } : {}) });
  }
