...
export class SessionAdministrationWorkflow implements SessionAdministrationWorkflowPort {
  ...
  private async transitionAndPublish(binding: Binding, transition: import("../domain/pane-thread-lifecycle.js").SessionTransition, type: "BindingDraining" | "BindingArchived", reason: string): Promise<Binding> {
    ...
    const next = this.options.store.transitionBindingWithOutbox({ id: binding.id, transition, event, view, messageId: binding.statusMessageId, card: renderProjectEntryCard(view, { lastActivityAt: binding.lastActivityAt }) }); this.options.outboundWork.wake(); await this.options.lifecycleEvents.publish(event); return next;
  }
