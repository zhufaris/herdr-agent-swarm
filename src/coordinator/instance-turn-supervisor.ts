...
interface Options { store: InstanceTurnSupervisionStore; paneHost: PaneHost; observer?: Pick<WorkerTurnObservationPort, "recover">; wake(instanceId: string): void; wakeOutbound?: () => void; convergeWorkerTurn?: (turnId: string) => void; presentation: Pick<WorkerPresentation, "workerTurn" | "workerHumanReviewNotification">; logger?: Pick<Logger, "info" | "warn"> }
...
export class InstanceTurnSupervisor {
  ...
  prepareRecovery(): void {
    ...
    for (const turnId of this.options.store.listActionableWorkerTurnCardIds()) this.options.convergeWorkerTurn?.(turnId);
  }
  ...
  private transition(turnId: string, generation: number, state: Parameters<InstanceTurnSupervisionStore["updateInstanceTurn"]>[0]["state"], eventKind: Parameters<InstanceTurnSupervisionStore["updateInstanceTurn"]>[0]["eventKind"], change: WorkerTurnCardChange, error: string | null = null, result: string | null = null): void {
    if (this.options.store.loadWorkerTurnCard(turnId)) {
      ...
      if (projected) {
        if (projected.projectionChanged || projected.notification.outcome === "reserved") { this.options.wakeOutbound?.(); this.options.convergeWorkerTurn?.(turnId); }
