...
import type { Binding, HerdrPane, ProjectConfig, ReconciliationPassResult } from "../domain/types.js";
...
import type { PriorityReconciliationScope } from "../runtime/priority-reconciliation-runner.js";
...
export interface BindingReconciliationPassResult extends ReconciliationPassResult {
  snapshotDurationMs: number;
  missingPaneDurationMs: number;
  existingBindingDurationMs: number;
  discoveryDurationMs: number;
  existingBindingCount: number;
  discoveryCandidateCount: number;
}

export interface BindingReconciliationPassOptions {
  projects: readonly ProjectConfig[];
  store: RuntimeReconciliationStore;
  herdr: HerdrPort;
  lifecycleEvents: LifecycleEventPublisher;
  channelPublisher: { enqueueRunCardUpdate(bindingId: string, promptId: string, messageId: string, viewVersion: number, cardRole: "answer", card: object): Promise<void> };
  wakeOutbound?: () => void;
...
export class BindingReconciliationPass {
  ...
  constructor(private readonly options: BindingReconciliationPassOptions) {
  ...
  async execute(scope: PriorityReconciliationScope): Promise<BindingReconciliationPassResult | void> {
    if (scope.kind === "panes") return this.reconcilePanes(scope.ids);
    return this.reconcileOnce(scope.kind === "workspaces" ? new Set(scope.ids) : undefined);
  }
  ...
  private async reconcileOnce(requestedWorkspaceIds?: ReadonlySet<string>): Promise<BindingReconciliationPassResult> {
    ...
    const workspaceIds = requestedWorkspaceIds ? [...requestedWorkspaceIds].filter((workspaceId) => reconciliationWorkspaceIds.has(workspaceId)) : [...reconciliationWorkspaceIds];
    ...
    const activeBindings = requestedWorkspaceIds ? allActiveBindings.filter((binding) => requestedWorkspaceIds.has(binding.workspaceId)) : allActiveBindings;
      ...
      try {
        ...
        if (pane.agentState === "unknown") {
          try {
            ...
            this.options.logger.debug({ event: "binding-runtime-observed", bindingId: binding.id, paneId: pane.paneId, agentState: observation.pane?.agentState ?? "unknown", evidenceSource: observation.evidenceSource }, "enriched bound pane from runtime evidence");
          } catch (error) {
    ...
    return { reconciledWorkspaceIds: new Set(panesByWorkspace.keys()), failures, snapshotDurationMs, missingPaneDurationMs, existingBindingDurationMs, discoveryDurationMs, existingBindingCount: existingPanes.length, discoveryCandidateCount: discoveryPanes.length };
  }
