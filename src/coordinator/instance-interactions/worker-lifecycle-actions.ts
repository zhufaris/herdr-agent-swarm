...
export class WorkerLifecycleActions {
  ...
  async handle(action: IncomingLarkCardAction, value: Record<string, unknown>): Promise<LarkCardActionResult> {
    ...
    if (conversationKey.startsWith("binding:") && !this.options.context.isCurrentBindingCard(value, conversationKey, action.chatId)) return warning("话题上下文已变化，请重新打开实例目录。");
    ...
    if (!instance || instance.generation !== Number(value.generation)) return warning("实例状态已变化，请刷新后重试。");
    ...
    if (value.action === "instance_open") return { card: this.options.views.detail(instance, conversationKey) };
    ...
    if (value.action === "instance_start") { await this.options.control.start({ actor, instanceId: instance.id }); return { toast: { type: "success", content: "实例启动已完成。" } }; }
    if (value.action === "instance_stop") { await this.options.control.stop({ actor, instanceId: instance.id }); return { toast: { type: "success", content: "实例已停止，worktree 已保留。" } }; }
  ...
  private openCardTarget(value: Record<string, unknown>, chatId: string): LarkCardActionResult {
    ...
    if (kind === "worker-turn") {
      ...
      if (!view || view.instanceGeneration !== generation || view.messageId !== messageId || !instance || instance.role !== "worker" || instance.workerSessionGeneration !== view.workerSessionGeneration || !instance.parent || !this.isOwnedCardBinding(value, chatId, instance.parent.bindingId, instance.parent.bindingGeneration ?? 1, instance.parent.paneId)) return warning("Worker Task 卡片已过期或不属于当前 Primary。");
      return { card: this.options.presentation.workerTurn(view) };
    }
...
function bindingCardContext(value: Record<string, unknown>): { bindingId?: string; bindingGeneration?: number } { return typeof value.bindingId === "string" && Number.isInteger(Number(value.bindingGeneration)) ? { bindingId: value.bindingId, bindingGeneration: Number(value.bindingGeneration) } : {}; }
