...
export const paneDirectoryCardActionNames = ["pane_primary_thread_forward", "pane_worker_thread_forward"] as const;
...
export function parseCardActionCommand(value: unknown, option?: string | null): CardActionCommand {
  ...
  if (action === "pane_primary_thread_forward") { const bindingId = string(item.bindingId); const bindingGeneration = integer(item.bindingGeneration); const paneId = string(item.paneId); const sourceMainMessageId = string(item.sourceMainMessageId); return bindingId && bindingGeneration !== null && paneId && sourceMainMessageId ? { kind: "pane-directory", action, bindingId, bindingGeneration, paneId, sourceMainMessageId } : unknown(); }
  if (action === "pane_worker_thread_forward") {
    const instanceId = string(item.instanceId); const generation = integer(item.generation); const workerSessionGeneration = integer(item.workerSessionGeneration);
    const bindingId = string(item.bindingId); const bindingGeneration = integer(item.bindingGeneration); const parentPaneId = string(item.parentPaneId); const sourceMainMessageId = string(item.sourceMainMessageId);
    return instanceId && generation !== null && workerSessionGeneration !== null && bindingId && bindingGeneration !== null && parentPaneId && sourceMainMessageId
      ? { kind: "pane-directory", action, instanceId, generation, workerSessionGeneration, bindingId, bindingGeneration, parentPaneId, sourceMainMessageId } : unknown();
  }
