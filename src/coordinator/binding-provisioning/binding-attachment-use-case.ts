...
export class BindingAttachmentUseCase {
  ...
  constructor(private readonly options: {
    config: BridgeConfig; store: BindingProvisioningStore; herdr: HerdrPort; projectsById: ReadonlyMap<string, ProjectConfig>; scheduler: PromptWorkScheduler;
    ...
    for (const project of options.config.projects) { if (!project.spaceName) continue; const projects = this.projectsBySpaceName.get(project.spaceName) ?? []; projects.push(project); this.projectsBySpaceName.set(project.spaceName, projects); }
  }

  async attach(message: IncomingLarkMessage, spaceName: string, paneReference: string): Promise<boolean> {
    const { config, store, herdr } = this.options; const projects = this.projectsBySpaceName.get(spaceName) ?? [];
  ...
  private async recoverFailedResetBinding(binding: Binding, pane: HerdrPane, actorOpenId: string): Promise<Binding> {
    ...
    const project = binding.projectId ? this.options.projectsById.get(binding.projectId) : undefined; const observation = await this.options.herdr.observeRuntime(pane.paneId);
