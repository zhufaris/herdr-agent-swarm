...
export class OutboundDeliveryExecutor {
  ...
  private async deliverGroupCardCreate(claim: OutboundDeliveryClaim, payload: string): Promise<void> {
    const reply = claim.reply;
    if (!reply.targetChatId || (reply.threadAliasId === null) === (reply.workerThreadId === null)) throw new Error("Group card target is incomplete");
    const sent = await this.lark.createTopic(JSON.parse(payload) as object, reply.idempotencyKey, reply.targetChatId);
  ...
  private async deliverReply(claim: OutboundDeliveryClaim, payload: string): Promise<void> {
    ...
    const sent = reply.kind === "text" ? await this.lark.replyText(rootMessageId, payload, reply.idempotencyKey) : await this.lark.replyCard(rootMessageId, JSON.parse(payload) as object, reply.idempotencyKey);
...
function requireRootMessageId(reply: OutboundReply): string { if (!reply.rootMessageId) throw new Error(`Outbound reply ${reply.id} has no root message target`); return reply.rootMessageId; }
