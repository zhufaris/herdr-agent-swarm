...
export function createPrimaryRuntime(options: {
  ...
  const externalTurns = new ExternalTurnObserver({
    store: stores.externalTurns, transcriptReader, bus, outboundWork, logger, presentation: cardKitPrimaryPresentation,
  ...
  const promptRun = new PromptRunWorkflow({
    store: stores.promptRun, herdr, bus, scheduler, outboundWork, logger, presentation: cardKitPrimaryPresentation, turnTimeoutMs: config.turnTimeoutMs,
