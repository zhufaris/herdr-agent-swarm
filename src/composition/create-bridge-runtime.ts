...
export function createBridgeRuntime(config: BridgeConfig, stores: SqliteStoreBundle, logger: Logger, availability: AgentRuntimeAvailability) {
  ...
  const turnControl = new TurnControlWorkflow({ store: stores.turnControl, herdr, idFactory: randomUUID, wakeOutbound: () => wakeTurnControlOutbound?.(), wakePrimary: (bindingId) => wakePriorityPrimary?.(bindingId), wakeInstance: (instanceId) => wakePriorityInstance?.(instanceId), maxQueueDepth: config.maxQueueDepth });
  ...
  const outbound = new OutboundIntentWriter(stores.outboundIntent, outboundWork); const transcriptReader = new TraexTranscriptReader({ sessionsRoot: config.traex.sessionsRoot });
  let instanceWork!: InstanceWorkScheduler; const workerTurns = new WorkerTurnObserver({ store: stores.instance, transcriptReader, wakeInstance: (instanceId) => instanceWork.wake(instanceId), wakeOutbound: () => outboundWork.wake() });
  instanceWork = new InstanceWorkScheduler({ store: stores.instance, drivers: agentDrivers, observer: workerTurns, wakeOutbound: () => outboundWork.wake(), logger });
  ...
  const cardContextRebuilder = new CardContextRebuilder(stores.cardContext, () => outboundWork.wake(), logger, outboundWork);
  const projector = new ConversationViewProjector(bus, stores.projection, outbound, channelPublisher, logger, answerPages, mainCards, { cardUpdateDebounceMs: config.runtimeTuning.cardUpdateDebounceMs }); const queueFeedbackProjector = new QueueFeedbackProjector({ store: stores.queueFeedback, outboundWork, logger }); channelPublisher.connectPromptScheduler(scheduler);
  let promptRun!: PromptRunWorkflow; const externalTurns = new ExternalTurnObserver({ store: stores.externalTurns, transcriptReader, bus, outboundWork, logger, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId), wakePrompt: (bindingId) => scheduler.wake({ kind: "prompt-ready", bindingId }) });
  promptRun = new PromptRunWorkflow({ store: stores.promptRun, herdr, bus, scheduler, outboundWork, logger, turnTimeoutMs: config.turnTimeoutMs, transcriptReader, mainCards, handoffExternalTurns: (bindingId) => externalTurns.handoff(bindingId), observeSupersedingExternalTurn: (binding, prompt, observation) => externalTurns.observeSupersedingTurn(binding, prompt, observation), recoverExternalTurns: (binding, prompt) => externalTurns.recoverAfterDetachedTurn(binding, prompt) });
  wakePriorityPrimary = (bindingId) => scheduler.wake({ kind: "prompt-ready", bindingId });
  const retiredPaneCleanup = new RetiredPaneCleanupWorkflow({ store: stores.retiredPaneCleanup, herdr, logger }); const provisioning = new BindingProvisioningWorkflow({ config, store: stores.bindingProvisioning, herdr, lark, lifecycleEvents: bus, outbound, outboundWork, immediateOutbound: channelPublisher, scheduler, primaryTools: primaryToolGateway, wakeRetiredPaneCleanup: () => void retiredPaneCleanup.requestScan(), logger }); const modelSelection = new ModelSelectionWorkflow({ config, store: stores.modelSelection, herdr, outbound, outboundWork, scheduler, mainCards, activeTurn: (bindingId) => promptRun.activeTurn(bindingId), logger }); const paneControl = new PaneControlWorkflow({ store: stores.paneControl, outbound, scheduler, model: modelSelection, turnControl, activeTurn: (bindingId) => promptRun.activeTurn(bindingId) }); const operationsQuery = new OperationsQueryWorkflow({ config, store: stores.operationsQuery, herdr, outbound, logger }); const sessionAdministration = new SessionAdministrationWorkflow({ config, store: stores.sessionAdministration, herdr, lifecycleEvents: bus, outbound, outboundWork, scheduler, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId) }); const deliveryRecovery = new DeliveryRecoveryWorkflow({ store: stores.deliveryRecovery, lark, outbound, outboundWork, logger }); const paneClosure = new PaneClosureWorkflow({ config, store: stores.paneClose, herdr, lifecycleEvents: bus, outbound, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId) }); const paneRetention = new PaneRetentionWorkflow({ projects: config.projects, store: stores.paneRetention, herdr, outbound, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId), logger });
  const sessionOperations = new SessionOperationWorkflow({ store: stores.sessionOperations, sessionAdministration, provisioning, paneControl, paneClosure, logger }); const cardInteractions = new CardInteractionWorkflow({ store: stores.cardInteraction, adminOpenIds: config.lark.adminOpenIds, sessionAdministration, sessionOperations, wakePrompt: (bindingId) => scheduler.wake({ kind: "prompt-ready", bindingId }), logger });
  ...
  const swarmCommands = new SwarmCommandGateway({ store: stores.commandIntents, resolver: commandResolver, outbound, logger, provisioning, modelSelection, paneControl, operationsQuery, sessionAdministration, paneClosure, promptRun, instanceControl });
  const instanceInteractions = new InstanceInteractionWorkflow({ projects: config.projects, adminOpenIds: config.lark.adminOpenIds, store: stores.instance, control: instanceControl, messaging: instanceMessaging, drivers: agentDrivers, outbound, workerCreation: swarmCommands });
  const startupViews = new StartupViewConverger(config, stores.promptAcceptance, outbound, outboundWork, answerPages, mainCards, logger); const inboundDispatcher = new InboundMessageDispatcher({ chatId: config.lark.chatId, allowedOpenIds: config.lark.allowedOpenIds, store: stores.inboundDispatch, inboundWork, logger }); const messageRouting = new InboundMessageRoutingWorkflow({ config, store: stores.inboundMessages, lifecycleEvents: bus, outbound, outboundWork, logger, scheduler, promptRun, provisioning, swarmCommands, instanceInteractions }); const cardActionRouter = new CardActionRouter({ chatId: config.lark.chatId, allowedOpenIds: config.lark.allowedOpenIds, adminOpenIds: config.lark.adminOpenIds, projects: config.projects, store: stores.inboundRouting, provisioning, cardInteractions, modelSelection, deliveryRecovery, instanceInteractions, logger, enqueueInitialPrompt: (binding, selection) => messageRouting.enqueueInitialProjectPrompt(binding, selection) }); const startupRecovery = new StartupRecoveryWorkflow({ config, store: stores.startupRecovery, herdr, lark, logger, scheduler, inboundWork, inboundDispatcher, cardActionRouter, messageRouting, promptRun, provisioning, paneControl, paneClosure, sessionOperations, swarmCommands, reconciler, retiredPaneCleanup, startupViews }); const coordinator = new InboundRouter({ lark, modelSelection, promptRun, reconciler, retiredPaneCleanup, sessionOperations, swarmCommands, inboundDispatcher, cardActionRouter, startupRecovery });
