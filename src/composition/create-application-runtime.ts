...
export function createApplicationRuntime(options: {
  ...
  const modelSelection = new ModelSelectionWorkflow({ config, store: stores.modelSelection, herdr, outbound, outboundWork, scheduler, mainCards, activeTurn: (bindingId) => promptRun.activeTurn(bindingId), presentation: cardKitApplicationPresentation, logger });
  const paneControl = new PaneControlWorkflow({ store: stores.paneControl, outbound, presentation: cardKitPanePresentation, scheduler, model: modelSelection, turnControl, activeTurn: (bindingId) => promptRun.activeTurn(bindingId) });
  const operationsQuery = new OperationsQueryWorkflow({ config, store: stores.operationsQuery, herdr, outbound, presentation: cardKitApplicationPresentation, logger });
  const sessionAdministration = new SessionAdministrationWorkflow({ config, store: stores.sessionAdministration, herdr, lifecycleEvents: bus, outbound, outboundWork, scheduler, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId), presentation: cardKitApplicationPresentation });
  const deliveryRecovery = new DeliveryRecoveryWorkflow({ store: stores.deliveryRecovery, lark, outbound, outboundWork, presentation: cardKitApplicationPresentation, logger });
  const paneClosure = new PaneClosureWorkflow({ config, store: stores.paneClose, herdr, lifecycleEvents: bus, outbound, presentation: cardKitPanePresentation, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId) });
  const paneRetention = new PaneRetentionWorkflow({ projects: config.projects, store: stores.paneRetention, herdr, outbound, presentation: cardKitPanePresentation, isBindingBusy: (bindingId) => promptRun.isBindingBusy(bindingId), logger });
  ...
  const cardInteractions = new CardInteractionWorkflow({ store: stores.cardInteraction, adminOpenIds: config.lark.adminOpenIds, sessionAdministration, sessionOperations, wakePrompt: (bindingId) => scheduler.wake({ kind: "prompt-ready", bindingId }), presentation: cardKitApplicationPresentation, logger });
  ...
  const swarmCommands = new SwarmCommandGateway({ store: stores.commandIntents, resolver: commandResolver, outbound, logger, provisioning, modelSelection, paneControl, operationsQuery, sessionAdministration, paneClosure, promptRun, instanceControl, presentation: cardKitApplicationPresentation });
  const instanceInteractions = new InstanceInteractionWorkflow({ projects: config.projects, adminOpenIds: config.lark.adminOpenIds, store: stores.instance, control: instanceControl, messaging: instanceMessaging, drivers: agentDrivers, outbound, workerCreation: swarmCommands, presentation: cardKitApplicationPresentation });
  const startupViews = new StartupViewConverger(config, stores.promptAcceptance, outbound, outboundWork, cardKitPrimaryPresentation, answerPages, mainCards, logger);
  ...
  const messageRouting = new InboundMessageRoutingWorkflow({ config, store: stores.inboundMessages, lifecycleEvents: bus, outbound, outboundWork, logger, scheduler, presentation: cardKitPrimaryPresentation, promptRun, provisioning, swarmCommands, instanceInteractions });
