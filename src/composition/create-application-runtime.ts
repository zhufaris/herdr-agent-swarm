...
export function createApplicationRuntime(options: {
  ...
  const swarmCommands = new SwarmCommandGateway({ store: stores.commandIntents, resolver: commandResolver, outbound, logger, provisioning, modelSelection, paneControl, operationsQuery, sessionAdministration, paneClosure, promptRun, instanceControl, presentation: presentation.application });
  const instanceInteractions = new InstanceInteractionWorkflow({ projects: config.projects, adminOpenIds: config.lark.adminOpenIds, store: stores.instance, control: instanceControl, messaging: instanceMessaging, drivers: agentDrivers, outbound, workerCreation: swarmCommands, presentation: presentation.application });
  const startupViews = new StartupViewConverger(config, stores.promptAcceptance, outbound, outboundWork, presentation.primary, answerPages, mainCards, logger);
  ...
  const cardActionRouter = new CardActionRouter({ chatId: config.lark.chatId, allowedOpenIds: config.lark.allowedOpenIds, adminOpenIds: config.lark.adminOpenIds, projects: config.projects, store: stores.inboundRouting, provisioning, cardInteractions, modelSelection, deliveryRecovery, instanceInteractions, logger, enqueueInitialPrompt: (binding, selection) => messageRouting.enqueueInitialProjectPrompt(binding, selection) });
