...
export function renderWorkerMainCard(view: WorkerMainView): object {
  const elements: object[] = [
    { tag: "markdown", content: `**Owner**  ${safe(view.ownerName)}  ·  **Primary pane**  \`${safe(view.parentPaneId)}\`\n**Session**  ${view.workerSessionGeneration}  ·  **Runtime**  ${RUNTIME_LABEL[view.runtimeState]}  ·  **Runtime gen**  ${view.runtimeGeneration}` },
...
function currentTaskContent(task: WorkerMainTaskSummary | null): string {
  return task ? `**Current Task**\n${safe(task.title)}  ·  ${PHASE_LABEL[task.phase]}${task.durationSeconds === null ? "" : `  ·  ${formatDuration(task.durationSeconds)}`}` : "**Current Task**\nNo active task";
}

function taskLine(task: WorkerMainTaskSummary): string {
  return `- ${PHASE_LABEL[task.phase]}  ${safe(task.title)}${task.durationSeconds === null ? "" : `  ${formatDuration(task.durationSeconds)}`}`;
}
