...
export function renderInstanceCreateCard(input: { projectId: string; requestedBy: string; conversationKey?: string }): object {
  return card("创建 Agent 实例", "blue", [{ tag: "form", name: "instance_create_form", elements: [
    ...
    formSubmitButton("创建实例", "instance_create_submit", { action: "instance_create_submit", projectId: input.projectId, requestedBy: input.requestedBy, ...(input.conversationKey ? { conversationKey: input.conversationKey } : {}) }, "primary")
  ] }]);
}

export function renderInstanceSteerCard(input: { instance: AgentInstance; requestedBy: string; conversationKey?: string }): object {
  return card(`Steer · ${input.instance.name}`, "blue", [
    ...
    { tag: "form", name: "instance_steer_form", elements: [
      ...
      formSubmitButton("发送 Steer", "instance_steer_submit", { action: "instance_steer_submit", instanceId: input.instance.id, generation: input.instance.generation, requestedBy: input.requestedBy, ...(input.conversationKey ? { conversationKey: input.conversationKey } : {}) }, "primary")
    ] }
  ]);
}

export function renderInstanceRemovalPlanCard(input: { instance: AgentInstance; workspace: WorkspaceLease; plan: InstanceRemovalPlan; requestedBy: string; conversationKey?: string }): object {
