...
export function renderInstanceDetailCard(input: { instance: AgentInstance; workspace: WorkspaceLease; capabilities: AgentCapabilities; turns: InstanceTurn[]; queueDepth: number; conversationKey?: string }): object {
  ...
  const context = input.conversationKey ? { conversationKey: input.conversationKey } : {};
  ...
  if (input.instance.role !== "primary") controls.push(callbackButton("设为 Primary", { action: "instance_set_primary", instanceId: input.instance.id, generation: input.instance.generation, ...context }));
