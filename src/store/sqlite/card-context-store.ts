...
export class SqliteCardContextStore {
  ...
  loadWorkerMainProjectionSource(workerId: string, generation: number): WorkerMainProjectionSource | null {
    ...
    const cards = (this.database.prepare("SELECT * FROM worker_turn_cards WHERE instance_id = ? AND worker_session_generation = ? ORDER BY created_at DESC, turn_id DESC").all(workerId, generation) as Array<Record<string, unknown>>).map(mapWorkerTurnCard).filter((value): value is WorkerTurnCardView => value !== null);
    const active = ["blocked", "running", "preparing"].flatMap((phase) => cards.filter((card) => card.phase === phase)).at(0) ?? cards.filter(({ phase }) => phase === "queued").at(-1) ?? null;
    const summary = (view: WorkerTurnCardView) => ({ turnId: view.turnId, title: summarizeTaskTitle(view.requestText), phase: view.phase, durationSeconds: durationSeconds(view.startedAt, view.finishedAt), taskCard: { aggregateKind: "worker-turn" as const, aggregateId: view.turnId, generation: view.instanceGeneration, messageId: view.messageId }, updatedAt: view.updatedAt });
    return {
  ...
  loadPrimaryWorkerActivity(promptId: string, generation: number): PrimaryWorkerActivitySummary[] {
    ...
    const rows = this.database.prepare(`SELECT c.* FROM worker_turn_cards c JOIN instance_turns t ON t.id = c.turn_id JOIN agent_instances worker ON worker.id = c.instance_id WHERE json_extract(t.actor_json, '$.kind') = 'thread-primary' AND json_extract(t.actor_json, '$.parentPromptId') = ? AND json_extract(t.actor_json, '$.bindingId') = ? AND json_extract(t.actor_json, '$.bindingGeneration') = ? AND worker.role = 'worker' AND worker.parent_binding_id = ? AND worker.parent_binding_generation = ? AND worker.parent_pane_id = ? AND worker.worker_session_generation = c.worker_session_generation ORDER BY c.updated_at DESC, c.turn_id`).all(promptId, run.bindingId, generation, run.bindingId, generation, binding.paneId) as Array<Record<string, unknown>>;
