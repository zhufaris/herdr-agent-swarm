...
export class SqliteBindingProjectionStore {
  ...
  recoverOrphanBindingWithProjection(input: RecoverOrphanBindingProjectionInput): RecoverOrphanBindingProjectionResult {
    return this.context.transaction(() => {
      ...
      const nativeSessionMatches = !persisted || Boolean(observed && persisted.source === observed.source && persisted.agent === observed.agent && persisted.kind === observed.kind && persisted.value === observed.value);
...
function isTraexCompatibleNativeAgent(pane: RecoverOrphanBindingProjectionInput["pane"]): boolean { return pane.agentKind !== null && pane.agentKind !== undefined && TRAEX_COMPATIBLE_AGENT_KINDS.has(pane.agentKind); }
