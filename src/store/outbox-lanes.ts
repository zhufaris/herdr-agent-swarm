...
export function outboundLaneKeySql(workerTurnIdExpression = "worker_turn_id"): string {
  return `CASE WHEN ${workerTurnIdExpression} IS NOT NULL AND selection_id = 'worker-progress' THEN 'worker-progress:' || ${workerTurnIdExpression} WHEN ${workerTurnIdExpression} IS NOT NULL THEN 'worker-turn:' || ${workerTurnIdExpression} WHEN card_role = 'answer' AND prompt_id IS NOT NULL THEN 'answer:' || prompt_id WHEN kind IN ('stream_content','stream_finish') THEN 'stream:' || root_message_id WHEN kind IN ('card_reply','text') THEN 'reply:' || id ELSE 'message:' || root_message_id END`;
}

export function outboundLaneKey(input: { id?: string; cardRole?: RequestCardRole | null; promptId?: string | null; workerTurnId?: string | null; selectionId?: string | null; kind: OutboundReplyKind; rootMessageId: string }): string {
