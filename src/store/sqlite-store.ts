...
export class SqliteBindingStore implements BindingStorePort, TurnControlStore {
  ...
  recoverRunningPrompts(): number {
    ...
    try {
      ...
      const safeModelClaims = this.database.prepare(`
        SELECT p.id
        FROM prompt_jobs p
        JOIN bindings b ON b.id = p.binding_id
        JOIN binding_model_preferences preference
          ON preference.binding_id = p.binding_id
          ...
          AND preference.dispatch_prompt_id = p.id
        WHERE p.state = 'running' AND p.observation_state = 'not_started'
          AND p.model_name IS NOT NULL AND p.model_revision IS NOT NULL
          AND preference.state = 'applying' AND preference.prepared_operation_id IS NULL
      `).all() as Array<{ id: string }>;
      this.database.prepare(`
        UPDATE binding_model_preferences
        SET state = 'pending', dispatch_prompt_id = NULL, prepared_operation_id = NULL, updated_at = ?
        WHERE state = 'applying' AND prepared_operation_id IS NULL
          AND dispatch_prompt_id IN (${safeModelClaims.length > 0 ? safeModelClaims.map(() => "?").join(", " ) : "NULL"})
      `).run(timestamp, ...safeModelClaims.map((prompt) => prompt.id));
      if (safeModelClaims.length > 0) {
        ...
        this.database.prepare(`
          ...
          WHERE id IN (${placeholders}) AND state = 'running' AND observation_state = 'not_started'
        `).run(timestamp, ...safeModelClaims.map((prompt) => prompt.id));
      }
      this.database.prepare(`
        UPDATE binding_model_preferences
        SET state = 'uncertain', updated_at = ?
        WHERE state = 'applying' AND prepared_operation_id IS NOT NULL
          AND EXISTS (
            SELECT 1 FROM prompt_jobs p JOIN bindings b ON b.id = p.binding_id
            WHERE p.id = binding_model_preferences.dispatch_prompt_id
              AND p.binding_id = binding_model_preferences.binding_id
      ...
      `).run(timestamp);
      const undispatched = this.database.prepare(`
        SELECT id FROM prompt_jobs
        WHERE state = 'running' AND observation_state = 'not_started' AND model_name IS NULL AND model_revision IS NULL
      `).all() as Array<{ id: string }>;
      this.database.prepare(`
  ...
  markPromptObservationDetached(id: string, notice: string): void {
    ...
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const result = this.database.prepare("UPDATE prompt_jobs SET observation_state = 'detached', was_detached = 1, error = ?, updated_at = ? WHERE id = ? AND state = 'running' AND observation_state = 'attached'").run(notice, timestamp, id);
      if (result.changes > 0) this.database.prepare(`
  ...
  markModelPromptPrepared(input: { bindingId: string; bindingGeneration: number; promptId: string; revision: number; operationId: string }): boolean {
    const changed = this.database.prepare(`
      UPDATE binding_model_preferences SET prepared_operation_id = ?, updated_at = ?
      WHERE binding_id = ? AND binding_generation = ? AND desired_revision = ? AND dispatch_prompt_id = ? AND state = 'applying'
        AND (prepared_operation_id IS NULL OR prepared_operation_id = ?)
    `).run(input.operationId, now(), input.bindingId, input.bindingGeneration, input.revision, input.promptId, input.operationId);
  ...
  markModelPromptAccepted(input: { bindingId: string; bindingGeneration: number; promptId: string; revision: number; operationId: string; turnId: string }): boolean {
    ...
    try {
      ...
      const preference = this.database.prepare(`
        UPDATE binding_model_preferences SET effective_model = desired_model, effective_revision = desired_revision, state = 'effective', dispatch_prompt_id = NULL, prepared_operation_id = NULL, updated_at = ?
        WHERE binding_id = ? AND binding_generation = ? AND desired_revision = ? AND dispatch_prompt_id = ? AND prepared_operation_id = ? AND state = 'applying'
      `).run(timestamp, input.bindingId, input.bindingGeneration, input.revision, input.promptId, input.operationId);
      if (Number(preference.changes) !== 1) { this.database.exec("COMMIT"); return false; }
      const prompt = this.database.prepare("UPDATE prompt_jobs SET transcript_turn_id = ?, updated_at = ? WHERE id = ? AND binding_id = ? AND model_revision = ? AND state = 'running' AND (transcript_turn_id IS NULL OR transcript_turn_id = ?)")
        .run(input.turnId, timestamp, input.promptId, input.bindingId, input.revision, input.turnId);
  ...
  rollbackPreparedModelPrompt(input: { bindingId: string; bindingGeneration: number; promptId: string; revision: number; operationId: string }): boolean {
    ...
    try {
      ...
      const prompt = this.database.prepare(`
        UPDATE prompt_jobs SET dispatched_at = NULL, updated_at = ?
        WHERE id = ? AND binding_id = ? AND model_revision = ? AND state = 'running'
          AND observation_state = 'attached' AND transcript_turn_id IS NULL
      `).run(timestamp, input.promptId, input.bindingId, input.revision);
      if (Number(prompt.changes) !== 1) { this.database.exec("COMMIT"); return false; }
      const preference = this.database.prepare(`
        UPDATE binding_model_preferences SET state = 'pending', dispatch_prompt_id = NULL, prepared_operation_id = NULL, updated_at = ?
        WHERE binding_id = ? AND binding_generation = ? AND desired_revision = ? AND dispatch_prompt_id = ? AND prepared_operation_id = ? AND state = 'applying'
      `).run(timestamp, input.bindingId, input.bindingGeneration, input.revision, input.promptId, input.operationId);
  ...
  claimPromptTranscriptTurn(input: { promptId: string; bindingId: string; turnId: string; startedAt: string }): TranscriptTurnClaimOutcome {
    ...
    try {
      ...
      if (before.transcript_turn_id !== null && (before.transcript_turn_id !== input.turnId || before.transcript_turn_started_at !== null)) {
        this.database.exec("COMMIT");
      ...
      const result = this.database.prepare(`
        ...
        WHERE id = ? AND binding_id = ?
          ...
          AND (observation_state = 'attached' OR (observation_state = 'detached' AND transcript_turn_id = ? AND transcript_turn_started_at IS NULL))
          AND dispatched_at IS NOT NULL
          AND (transcript_turn_id IS NULL OR (transcript_turn_id = ? AND transcript_turn_started_at IS NULL))
      `).run(input.turnId, input.startedAt, timestamp, input.promptId, input.bindingId, input.turnId, input.turnId);
  ...
  adoptExternalTurn(input: Parameters<BindingStorePort["adoptExternalTurn"]>[0]): ExternalTurnAdoption {
    ...
    try {
      ...
      const owners = this.database.prepare("SELECT * FROM prompt_jobs WHERE transcript_turn_id = ? ORDER BY id LIMIT 2").all(input.turnId) as PromptRow[];
      ...
      for (const row of active) {
        this.database.prepare("UPDATE prompt_jobs SET state = 'failed', observation_state = 'completed', error = ?, updated_at = ? WHERE id = ?").run("Superseded by a newer external Herdr turn; prior outcome is uncertain", timestamp, row.id);
      ...
      if (candidates.length === 1) {
        ...
        this.database.prepare(`UPDATE prompt_jobs SET execution_origin = 'herdr', state = 'running', observation_state = 'attached', dispatched_at = ?, transcript_turn_id = ?, transcript_turn_started_at = ?, error = NULL, updated_at = ? WHERE id = ?`)
          .run(input.startedAt, input.turnId, input.startedAt, timestamp, promptId);
        const queuedView = this.loadRunCard(promptId)!;
        const runningView = reduceRunCard(queuedView, { type: "started", occurredAt: input.startedAt });
        outboxReserved = Number(this.database.prepare("UPDATE outbound_replies SET payload = ?, view_version = ?, updated_at = ? WHERE prompt_id = ? AND kind = 'stream_card_create' AND card_role = 'answer' AND state = 'pending'").run(JSON.stringify(input.answerCardFor(runningView)), runningView.viewVersion, timestamp, promptId).changes) > 0;
      } else {
        promptId = input.externalPromptId;
        outcome = "created_external";
        this.database.prepare(`INSERT INTO prompt_jobs(id, binding_id, lark_message_id, actor_open_id, body, execution_origin, dispatch_kind, state, observation_state, dispatched_at, transcript_turn_id, transcript_turn_started_at, attempt_count, created_at, updated_at) VALUES (?, ?, ?, 'herdr', ?, 'herdr', 'turn', 'running', 'attached', ?, ?, ?, 1, ?, ?)`)
          .run(promptId, input.bindingId, input.externalMessageId, input.requestText, input.startedAt, input.turnId, input.startedAt, input.startedAt, timestamp);
        ...
        this.insertRunCard(view);
        this.database.prepare(`INSERT INTO outbound_replies(id, idempotency_key, binding_id, prompt_id, view_version, card_role, root_message_id, kind, payload, lane_key, state, attempt_count, next_attempt_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 'answer', ?, 'stream_card_create', ?, ?, 'pending', 0, ?, ?, ?)`)
          .run(randomUUID(), `run-card:create:${promptId}:answer`, input.bindingId, promptId, runningView.viewVersion, binding.root_message_id, JSON.stringify(input.answerCardFor(runningView)), `answer:${promptId}`, timestamp, timestamp, timestamp);
        outboxReserved = true;
      }
  ...
  acceptClassifiedPrompt(input: ClassifiedPromptInput): ClassifiedPromptAcceptance {
    ...
    try {
      ...
      const binding = this.database.prepare("SELECT generation, last_agent_state, state, lifecycle, attachment FROM bindings WHERE id = ?").get(input.prompt.bindingId) as { generation: number; last_agent_state: string; state: string; lifecycle: string; attachment: string } | undefined;
      const parent = input.candidateParentPromptId === null ? undefined : this.database.prepare(`
        SELECT p.id, p.state, p.dispatch_kind, p.observation_state, p.was_detached, c.activity_at
        FROM prompt_jobs p LEFT JOIN run_cards c ON c.prompt_id = p.id
        WHERE p.id = ? AND p.binding_id = ?
      `).get(input.candidateParentPromptId, input.prompt.bindingId) as { id: string; state: string; dispatch_kind: string; observation_state: string; was_detached: number; activity_at: string | null } | undefined;
      const runningOrdinary = Number((this.database.prepare("SELECT COUNT(*) AS count FROM prompt_jobs WHERE binding_id = ? AND state = 'running' AND dispatch_kind = 'turn'").get(input.prompt.bindingId) as { count: number }).count);
      ...
      this.database.prepare(`INSERT INTO prompt_jobs(id, binding_id, lark_message_id, actor_open_id, body, dispatch_kind, parent_prompt_id, steering_origin, source_prompt_id, was_detached, state, observation_state, attempt_count, error, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, 0, 'queued', 'not_started', 0, NULL, ?, ?)`)
        .run(input.prompt.id, input.prompt.bindingId, input.prompt.larkMessageId, input.prompt.actorOpenId, input.prompt.body, dispatchKind, parentPromptId, steeringOrigin, input.acceptedAt, input.acceptedAt);
      this.insertRunCard(view);
      this.database.prepare(`
        INSERT INTO outbound_replies(id, idempotency_key, binding_id, prompt_id, view_version, card_role, root_message_id, kind, payload, lane_key, state, attempt_count, next_attempt_at, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, 'answer', ?, 'stream_card_create', ?, ?, 'pending', 0, ?, ?, ?)
      `).run(randomUUID(), `run-card:create:${input.prompt.id}:answer`, input.prompt.bindingId, input.prompt.id, view.viewVersion, input.rootMessageId, JSON.stringify(input.answerCardFor(view)), `answer:${input.prompt.id}`, input.acceptedAt, input.acceptedAt, input.acceptedAt);
      const prompt = this.requirePrompt(input.prompt.id);
      const storedView = this.loadRunCard(input.prompt.id)!;
      this.database.exec("COMMIT");
      return { prompt, view: storedView, inserted: true, decision: automatic ? "automatic_steering" : "ordinary", fallbackReason };
    } catch (error) {
      if (this.database.isTransaction) this.database.exec("ROLLBACK");
      throw error;
    }
  ...
  claimNextDispatchablePrompt(bindingId: string): { binding: Binding; prompt: PromptJob; model: { name: string; revision: number } | null } | null {
    ...
    try {
      ...
      const preference = this.database.prepare("SELECT * FROM binding_model_preferences WHERE binding_id = ? AND binding_generation = ? AND state = 'pending'").get(bindingId, Number(bindingRow.generation)) as ModelPreferenceRow | undefined;
      if (preference) {
        const pinned = this.database.prepare("UPDATE prompt_jobs SET model_name = ?, model_revision = ? WHERE id = ? AND model_revision IS NULL")
          .run(preference.desired_model, preference.desired_revision, row.id);
        const applying = this.database.prepare("UPDATE binding_model_preferences SET state = 'applying', dispatch_prompt_id = ?, prepared_operation_id = NULL, updated_at = ? WHERE binding_id = ? AND binding_generation = ? AND desired_revision = ? AND state = 'pending'")
          .run(row.id, now(), bindingId, Number(bindingRow.generation), preference.desired_revision);
      ...
      this.database.exec("COMMIT");
      return { binding: mapBinding(bindingRow), prompt: mapPrompt(promptRow), model: preference ? { name: preference.desired_model, revision: Number(preference.desired_revision) } : null };
    } catch (error) { this.database.exec("ROLLBACK"); throw error; }
  }
  ...
  enqueueOutboundReply(input: Omit<OutboundReply, "laneKey" | "promptId" | "workerTurnId" | "workerId" | "workerSessionGeneration" | "viewVersion" | "cardSequence" | "selectionId" | "cardRole" | "targetRole" | "state" | "attemptCount" | "error" | "deliveredMessageId" | "cardIdCheckpoint" | "failureClass" | "httpStatus" | "larkErrorCode" | "autoRecoveryCount" | "deadLetteredAt" | "nextAttemptAt" | "createdAt" | "updatedAt"> & { promptId?: string | null; workerTurnId?: string | null; workerId?: string | null; workerSessionGeneration?: number | null; viewVersion?: number | null; cardSequence?: number | null; selectionId?: string | null; cardRole?: OutboundReply["cardRole"]; targetRole?: OutboundReply["targetRole"]; laneKeyOverride?: string }): OutboundReply {
    ...
    try {
      ...
      if (input.kind === "card_update" && input.workerId && input.workerSessionGeneration !== undefined && input.workerSessionGeneration !== null && input.viewVersion !== undefined && input.viewVersion !== null) {
        this.database.prepare("DELETE FROM outbound_replies WHERE worker_id = ? AND worker_session_generation = ? AND kind = 'card_update' AND state = 'pending' AND COALESCE(view_version, 0) < ?")
          .run(input.workerId, input.workerSessionGeneration, input.viewVersion);
      }
  ...
  inspectIntegrity(limit: number): SqliteIntegrityInspection {
    return inspectSqliteIntegrity(this.database, limit);
  }
  ...
  private ensurePromptCancelledState(): void {
    ...
    const columns = new Set((this.database.prepare("PRAGMA table_info(prompt_jobs)").all() as Array<{ name: string }>).map((column) => column.name));
  ...
  private ensurePromptProvenanceColumns(): void {
    ...
    if (!names.has("steering_origin")) this.database.exec("ALTER TABLE prompt_jobs ADD COLUMN steering_origin TEXT CHECK(steering_origin IN ('explicit','automatic','converted'))");
  ...
  private ensureModelPreferenceSchema(): void {
    const promptColumns = new Set((this.database.prepare("PRAGMA table_info(prompt_jobs)").all() as Array<{ name: string }>).map((column) => column.name));
    if (!promptColumns.has("model_name")) this.database.exec("ALTER TABLE prompt_jobs ADD COLUMN model_name TEXT");
    if (!promptColumns.has("model_revision")) this.database.exec("ALTER TABLE prompt_jobs ADD COLUMN model_revision INTEGER CHECK(model_revision IS NULL OR model_revision >= 0)");
    this.database.exec(`
      CREATE TABLE IF NOT EXISTS binding_model_preferences(
        binding_id TEXT PRIMARY KEY REFERENCES bindings(id), binding_generation INTEGER NOT NULL CHECK(binding_generation >= 1),
        desired_model TEXT NOT NULL CHECK(length(desired_model) > 0), desired_revision INTEGER NOT NULL CHECK(desired_revision >= 1),
  ...
  private ensurePromptExecutionOriginColumn(): void {
    const names = new Set((this.database.prepare("PRAGMA table_info(prompt_jobs)").all() as Array<{ name: string }>).map((column) => column.name));
    if (!names.has("execution_origin")) this.database.exec("ALTER TABLE prompt_jobs ADD COLUMN execution_origin TEXT NOT NULL DEFAULT 'bridge' CHECK(execution_origin IN ('bridge','herdr'))");
    this.database.exec("CREATE INDEX IF NOT EXISTS prompt_jobs_transcript_turn ON prompt_jobs(transcript_turn_id) WHERE transcript_turn_id IS NOT NULL");
  }
  ...
  private ensureCardContextProjectionTables(): void {
    ...
    try {
      ...
      if (!columns.has("parent_binding_generation")) this.database.exec("ALTER TABLE agent_instances ADD COLUMN parent_binding_generation INTEGER");
      this.database.exec(`
  ...
  private ensureWorkerTurnContextReferences(): void {
    ...
    try {
      const columns = new Set((this.database.prepare("PRAGMA table_info(worker_turn_cards)").all() as Array<{ name: string }>).map(({ name }) => name));
      if (!columns.has("worker_main_ref_json")) this.database.exec("ALTER TABLE worker_turn_cards ADD COLUMN worker_main_ref_json TEXT");
      if (!columns.has("primary_answer_ref_json")) this.database.exec("ALTER TABLE worker_turn_cards ADD COLUMN primary_answer_ref_json TEXT");
      if (!columns.has("worker_session_generation")) this.database.exec("ALTER TABLE worker_turn_cards ADD COLUMN worker_session_generation INTEGER NOT NULL DEFAULT 1");
      this.database.prepare("INSERT INTO schema_migrations(version) VALUES (18)").run();
      this.database.exec("COMMIT");
    } catch (error) { if (this.database.isTransaction) this.database.exec("ROLLBACK"); throw error; }
  }
  ...
  private ensureWorkerMainOutboxIdentity(): void {
    ...
    try {
      const columns = new Set((this.database.prepare("PRAGMA table_info(outbound_replies)").all() as Array<{ name: string }>).map(({ name }) => name));
      if (!columns.has("worker_id")) this.database.exec("ALTER TABLE outbound_replies ADD COLUMN worker_id TEXT REFERENCES agent_instances(id) ON DELETE CASCADE");
      if (!columns.has("worker_session_generation")) this.database.exec("ALTER TABLE outbound_replies ADD COLUMN worker_session_generation INTEGER");
  ...
  private ensureRunCardProgressSummary(): void {
    const names = new Set((this.database.prepare("PRAGMA table_info(run_cards)").all() as Array<{ name: string }>).map((column) => column.name));
    if (!names.has("progress_summary_json")) this.database.exec(`ALTER TABLE run_cards ADD COLUMN progress_summary_json TEXT NOT NULL DEFAULT '{"total":0,"stepTotal":0,"stepDone":0}'`);
    const migrated = this.database.prepare("SELECT 1 FROM schema_migrations WHERE version = 6").get();
    if (migrated) return;
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const rows = this.database.prepare("SELECT prompt_id, progress_events_json FROM run_cards").all() as Array<{ prompt_id: string; progress_events_json: string }>;
      const update = this.database.prepare("UPDATE run_cards SET progress_events_json = ?, progress_summary_json = ? WHERE prompt_id = ?");
      for (const row of rows) {
      ...
      this.database.prepare("INSERT INTO schema_migrations(version) VALUES (6)").run();
      this.database.exec("COMMIT");
    } catch (error) { this.database.exec("ROLLBACK"); throw error; }
  }
  ...
  private ensureRunCardSessionTitle(): void {
    const columns = this.database.prepare("PRAGMA table_info(run_cards)").all() as Array<{ name: string }>;
    if (!columns.some((column) => column.name === "session_title")) this.database.exec("ALTER TABLE run_cards ADD COLUMN session_title TEXT");
  }
