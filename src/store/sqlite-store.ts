...
export class SqliteBindingStore implements BindingStorePort {
  ...
  inspectIntegrity(limit: number): SqliteIntegrityInspection {
    ...
    const quickRows = this.database.prepare(`PRAGMA quick_check(${boundedLimit + 1})`).all() as Array<Record<string, unknown>>;
    ...
    const foreignKeys = this.database.prepare(`SELECT "table", rowid FROM pragma_foreign_key_check LIMIT ${boundedLimit + 1}`).all() as Array<{ table: string; rowid: number | null }>;
    ...
    const rules: Array<{ rule: string; table: string; sql: string }> = [
      ...
      { rule: "prompt_parent_reference", table: "prompt_jobs", sql: "SELECT COUNT(*) AS count FROM prompt_jobs child LEFT JOIN prompt_jobs parent ON parent.id = child.parent_prompt_id WHERE child.parent_prompt_id IS NOT NULL AND parent.id IS NULL" },
      { rule: "multiple_running_turns", table: "prompt_jobs", sql: "SELECT COUNT(*) AS count FROM (SELECT binding_id FROM prompt_jobs WHERE state = 'running' AND dispatch_kind = 'turn' GROUP BY binding_id HAVING COUNT(*) > 1)" },
