...
export function inspectSqliteIntegrity(database: DatabaseSync, limit: number): SqliteIntegrityInspection {
  ...
  const rules: Array<{ rule: string; table: string; sql: string }> = [
    ...
    { rule: "outbound_group_target_invalid", table: "outbound_replies", sql: "SELECT COUNT(*) AS count FROM outbound_replies WHERE (kind = 'group_card_create' AND (root_message_id IS NOT NULL OR target_chat_id IS NULL OR ((thread_alias_id IS NOT NULL) = (worker_thread_id IS NOT NULL)))) OR (kind != 'group_card_create' AND (root_message_id IS NULL OR target_chat_id IS NOT NULL OR thread_alias_id IS NOT NULL OR worker_thread_id IS NOT NULL))" },
    { rule: "thread_alias_binding_mismatch", table: "binding_thread_aliases", sql: "SELECT COUNT(*) AS count FROM binding_thread_aliases a LEFT JOIN bindings b ON b.id = a.binding_id WHERE b.id IS NULL OR (a.state = 'active' AND (b.chat_id != a.chat_id OR b.generation != a.binding_generation OR b.pane_id IS NOT a.pane_id OR b.state != 'active' OR b.lifecycle != 'active' OR b.attachment != 'attached'))" },
    { rule: "worker_thread_owner_mismatch", table: "worker_session_threads", sql: "SELECT COUNT(*) AS count FROM worker_session_threads t LEFT JOIN agent_instances w ON w.id = t.worker_id LEFT JOIN bindings b ON b.id = t.parent_binding_id WHERE t.state = 'active' AND (w.id IS NULL OR b.id IS NULL OR w.role != 'worker' OR w.worker_session_lifecycle != 'active' OR w.worker_session_generation != t.worker_session_generation OR w.parent_binding_id IS NOT t.parent_binding_id OR w.parent_binding_generation IS NOT t.parent_binding_generation OR w.parent_pane_id IS NOT t.parent_pane_id OR b.chat_id != t.chat_id OR b.generation != t.parent_binding_generation OR b.pane_id IS NOT t.parent_pane_id OR b.state != 'active' OR b.lifecycle != 'active' OR b.attachment != 'attached')" },
