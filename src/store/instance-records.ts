...
export function mapAgentInstance(row: AgentInstanceRow): AgentInstance {
  ...
  return {
    ...
    model: row.model, sourcePrimaryPaneLabel: row.source_primary_pane_label, parent, workerSessionLifecycle: row.worker_session_lifecycle as WorkerSessionLifecycle | null, desiredState: row.desired_state as DesiredInstanceState, observedState: row.observed_state as ObservedInstanceState,
